...
class _AuthPageState extends State<AuthPage> with TickerProviderStateMixin {
  ...
  AuthBloc? _authBloc; // Cached AuthBloc reference
  ...
  void didChangeDependencies() {
    ...
    _authBloc = context.read<AuthBloc>(); // Cache AuthBloc
  }
  ...
  Future<void> _navigateBasedOnRole(String uid) async {
    try {
      ...
      if (!userDoc.exists) {
        ...
        return; // Delay navigation until profile is created
      }
  ...
  Future<void> _handleAuthButtonPress() async {
    if (!mounted || _isPhoneLinkingInProgress) return;
    try {
                      ...
                      if (_signupStep == 0) ...[
                        ...
                        _buildModernTextField(
                          ...
                          suffixIcon: IconButton(
                            ...
                            onPressed: () => setDialogState(
                                () => _obscurePhoneConfirmPassword = !_obscurePhoneConfirmPassword),
                          ),
                        ),
                        const SizedBox(height: 16),
                        _buildModernTextField(
                              ...
                              if (_signupStep < 3) {
                                if (_signupStep == 0) {
                                  ...
                                  if (phoneController.text.trim().isEmpty) {
                                    setDialogState(() {
                                      _signupStep = 2; // Skip phone verification if no phone provided
                                ...
                                try {
                                  final location = await _fetchUserLocation();
                                  await FirebaseFirestore.instance.collection('users').doc(uid).set({
                                    'email': _emailController.text.trim(), // Use email from signup
                                    'firstName': firstNameController.text.trim(),
                                    'lastName': lastNameController.text.trim(),
                                ...
                                children: [
                                  ...
                                  Expanded(
                                    child: TextField(
                                      ...
                                      keyboardType: TextInputType.phone,
                                      style: GoogleFonts.poppins(color: Colors.white),
                                      decoration: _modernInputDecoration('Phone Number', isPhone: true),
                                    ),
                                  ),
                                ],
                              ),
